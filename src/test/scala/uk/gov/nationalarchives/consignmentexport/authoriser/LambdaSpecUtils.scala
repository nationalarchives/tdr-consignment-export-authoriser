...
class LambdaSpecUtils extends AnyFlatSpec with BeforeAndAfterEach with BeforeAndAfterAll with TableDrivenPropertyChecks with Matchers with MockitoSugar with EitherValues {
  ...
  val wiremockKmsServer = new WireMockServer(new WireMockConfiguration().port(9002).extensions(new ResponseDefinitionTransformerV2 {
  ...
  def stubKmsResponse: StubMapping = wiremockKmsServer.stubFor(post(urlEqualTo("/")))
